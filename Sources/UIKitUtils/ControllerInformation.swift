...
public extension ControllerInformation {
    ...
    ///   - animated: 是否显示动画
    ///   - completion: 完成回调
    func dismissToController<T: UIViewController>(fromViewController: UIViewController, toViewController: T.Type, animated: Bool, completion: (() -> Void)? = nil) {
    ...
    ///   - animated: 是否显示动画
    ///   - completion: 完成回调
    func backToRootViewController(animated: Bool, completionHandler: (() -> Void)? = nil) {
        ...
        if vc?.presentingViewController != nil {
            ...
            if let navigationController = vc?.presentingViewController?.navigationController {
                vc?.dismiss(animated: false) {
                    navigationController.popToRootViewController(isAnimated: animated, completionHandler: completionHandler)
                }
            } else {
                vc?.dismiss(animated: animated, completion: completionHandler)
            }
        } else {
